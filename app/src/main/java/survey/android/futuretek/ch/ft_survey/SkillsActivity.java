...
public class SkillsActivity extends BaseActivity {
    ...
    protected void onResume() {
        ...
        //if selected skill value is 'None of the above', I'll request the new skill
    ...
    private void requestSkill(){
        ...
        openInputDialog(new View.OnClickListener() {
            public void onClick(View v) {
                EditText inputSkill = ((EditText) v.findViewById(R.id.userInput));
                try {
                    skills = getDatabase().get("skills");
                } catch (Exception e) {
                    e.printStackTrace();
                }
                if (inputSkill == null || inputSkill.isEmpty()) {
                    List<String> textArray = new ArrayList<String>(1);
                    textArray.add("Please add a skill");
                    animateText(textArray, new AnimationListDone() {
                    });
                } else {
                    insertSkill(inputSkill);
                }
            }
        });
    }
